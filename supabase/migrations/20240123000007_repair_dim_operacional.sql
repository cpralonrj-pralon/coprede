-- REPAIR SCRIPT 007: Fix dim_operacional and Recreate Views
-- Resolução do erro: column op.segmento_rede does not exist

do $$ 
begin
    -- 1. Ensure 'segmento_rede' exists in 'dim_operacional'
    if exists (select from information_schema.tables where table_name = 'dim_operacional') then
...
-- 2. Force Re-Creation of Views (Again, to succeed now that column exists)
